...
export async function getDashboardStats() {
  const response = await authenticatedFetch("/registro-base-ti/dashboard/stats")
  if (!response.ok) throw new Error("Error fetching dashboard stats")
  return response.json()
}

export async function getLisMasUsados() {
  const response = await authenticatedFetch("/registro-base-ti/dashboard/lis-mas-usados")
  if (!response.ok) throw new Error("Error fetching LIS más usados")
  return response.json()
}

export async function getLisPorRegiones() {
  const response = await authenticatedFetch("/registro-base-ti/dashboard/lis-por-regiones")
  if (!response.ok) throw new Error("Error fetching LIS por regiones")
  return response.json()
}

export async function getModulosMasUsados() {
  const response = await authenticatedFetch('/registro-base-ti/dashboard/modulos-mas-usados')
  if (!response.ok) {
...
export async function getLisPorHospitales() {
  const response = await authenticatedFetch('/registro-base-ti/dashboard/hospitales-por-lis')
  if (!response.ok) {
    const text = await response.text().catch(() => '')
...
export async function getHospitalesPorModuloAll() {
  const response = await authenticatedFetch('/registro-base-ti/dashboard/hospitales-por-modulo')
  if (!response.ok) {
    const text = await response.text().catch(() => '')
...
export async function getRegistroBaseTI() {
  const response = await authenticatedFetch("/registro-base-ti");
  if (!response.ok) {
...
export async function getRegistroBaseTIById(id: number) {
  const response = await authenticatedFetch(`/registro-base-ti/${id}`);
...
export async function getAreasMedicas() {
  const response = await authenticatedFetch("/area-medica");
  if (!response.ok) throw new Error("Error al obtener áreas médicas");
  return response.json();
}

export async function getLis() {
  const response = await authenticatedFetch("/lis");
...
export async function searchRegistroByLisAndModulos(lis_id: number, modulo_ids: number[]) {
  const response = await authenticatedFetch(`/registro-base-ti/search`, {
    method: 'POST',
...
export async function getModulosForRegistro(registro_base_id: number) {
  const response = await authenticatedFetch(`/registro-base-ti/${registro_base_id}/modulos`)
...
export async function getRegistroBaseTIWithModulos() {
  const response = await authenticatedFetch('/registro-base-ti/with-modulos')
  if (!response.ok) {
    const text = await response.text()
